...
Etude is a small suite of private, offline, browser-based tools for practicing musicians. No accounts, no uploads. Everything runs in the browser. The site uses anonymous, cookie-free page-view analytics (Cloudflare Web Analytics) — see `privacy.html`. Live at https://etude.fyi/ (local dir `~/code/etude`, GitHub remote `etude`).
